...
class RemakeRequirementViewsFixConflict extends Migration
{
    /**
     * Run the migrations.
     *
...
left join requirement_histories rh on rh.requirement_id = rq.id and rh.role_id = r.id and rh.valid = 1 and rh.id = (select max(rh2.id) from requirement_histories rh2 where rh2.requirement_id = rh.requirement_id and rh2.role_id = r.id) 
  and rh.completion_date + interval rq.renewal_period month > current_timestamp() and !exists(select 1 from requirement_history_reviews rhr where rhr.requirement_history_id = rh.id and rhr.status = \'declined\') 
...
,count(distinct rq.id) AS requirement_count,count(distinct rh.id) AS requirements_completed_count 
...
left join requirement_histories rh on rh.requirement_id = rq.id and rh.role_id = r.id and rh.valid = 1 and rh.id = (select max(rh2.id) from requirement_histories rh2 where rh2.requirement_id = rh.requirement_id and rh2.role_id = r.id) 
  ...
  and !exists(select 1 from requirement_history_reviews rhr where rhr.requirement_history_id = rh.id and rhr.status = \'declined\') 
where r.user_id is not null  and r.deleted_at is null 
...
select cho.contractor_id AS contractor_id,ho.id AS hiring_organization_id,ho.name AS name,count(distinct rq.id) AS requirement_count,count(distinct rh.id) AS requirements_completed_count 
...
select cho.contractor_id AS contractor_id,ho.id AS hiring_organization_id,ho.name AS hiring_organization_name,p.id AS position_id,p.name AS position_name,count(distinct rq.id) AS requirement_count,count(distinct rh.id) AS requirements_completed_count 
from contractor_hiring_organization cho
join positions p on p.is_active = 1 and p.position_type = \'contractor\'  and p.hiring_organization_id = cho.hiring_organization_id
join contractor_position cp on cp.contractor_id  = cho.contractor_id and cp.position_id = p.id
join position_requirement prs on prs.position_id = p.id 
...
select cho.contractor_id AS contractor_id,count(distinct rq.id) AS requirement_count,count(distinct rh.id) AS requirements_completed_count 
from contractor_hiring_organization cho 
join positions p on p.is_active = 1 and p.position_type = \'contractor\'  and p.hiring_organization_id = cho.hiring_organization_id
join contractor_position cp on cp.contractor_id  = cho.contractor_id and cp.position_id = p.id
join position_requirement prs on prs.position_id = p.id 
