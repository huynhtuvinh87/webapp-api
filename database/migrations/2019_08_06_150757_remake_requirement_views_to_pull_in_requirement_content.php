...
class RemakeRequirementViewsToPullInRequirementContent extends Migration
{
    /**
     * Run the migrations.
     *
...
,rq.id AS requirement_id,rc.name AS requirement_name, rc.lang AS lang, rq.type AS requirement_type,rq.content_type as requirement_content_type,case when current_timestamp() between rh.completion_date + interval rq.renewal_period month - interval rq.warning_period day and rh.completion_date + interval rq.renewal_period month then \'in_warning\' when rh.completion_date + interval rq.renewal_period month > current_timestamp() then \'on_time\' else \'past_due\' end AS requirement_status
,rh.completion_date AS completion_date,rh.completion_date + interval rq.renewal_period month AS due_date,rh.completion_date + interval rq.renewal_period month - interval rq.warning_period day as warning_date,ex.id AS exclusion_request_id,ex.status AS exclusion_status
...
where r.user_id is not null and r.deleted_at is null;');
    ...
    {
        Schema::table('pull_in_requirement_content', function (Blueprint $table) {
            //
        });
    }
}
