...
class UpdateRequirementViewsToIgnoreOldExclusionRequests extends Migration
{
    /**
     * Run the migrations.
     *
...
left outer join exclusion_requests ex on ex.requirement_id = rq.id and ex.contractor_id = cho.contractor_id and ex.id = (select max(ex2.id) from exclusion_requests ex2 where ex2.requirement_id = ex.requirement_id and ex2.contractor_id = ex.contractor_id )
...
left join exclusion_requests ex on ex.requirement_id = rq.id and ex.requester_role_id = r.id and ex.id = (select max(ex2.id) from exclusion_requests ex2 where ex2.requirement_id = ex.requirement_id and ex2.requester_role_id = ex.requester_role_id)
