...
class AddUploadDateToRequirementViews extends Migration
{
    /**
     * Run the migrations.
     *
...
,rq.id AS requirement_id,rq.name AS requirement_name,rq.type AS requirement_type,rq.content_type as requirement_content_type,case when current_timestamp() between rh.completion_date + interval rq.renewal_period month - interval rq.warning_period day and rh.completion_date + interval rq.renewal_period month then \'in_warning\' when rh.completion_date + interval rq.renewal_period month > current_timestamp() then \'on_time\' else \'past_due\' end AS requirement_status
...
where r.user_id is not null and r.deleted_at is null;');
    ...
    {
        Schema::table('requirement_views', function (Blueprint $table) {
            //
        });
    }
}
