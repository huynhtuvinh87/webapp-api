...
class NewContractorEmployeePositionTest extends TestCase
{
    ...
    public function testNotification()
    {
        try {
            ...
            if (config('app.env') != 'production') {
                $user->notify(new NewContractorEmployeePosition($this->position, $this->hiring_organization));
            }
            ...
            Notification::assertSentTo(
                $user,
                NewContractorEmployeePosition::class,
                function ($notification) use ($user) {
                    $notifiable = $notification->toMail($user);
                    $this->assertEquals("Re: New Employee Position Assigned to You!", $notifiable->subject);
